...
import { getPendingFormat, getAwaitingTableData } from "./formatPipeline/conversationState";
...
export class AIPMBot extends ActivityHandler {
  constructor() {
    ...
    this.onMessage(async (context: TurnContext, next) => {
      ...
      //    Skip when: pending format confirmation, awaiting table paste, or simple confirmation words.
      const isConfirmWord = /\b(yes|no|save|ja|nein|speichern|cancel|abbrechen)\b/i.test(text) && text.split(/\s+/).length <= 4;
      const hasPending = !!(await getPendingFormat(conversationId));
      const hasAwaiting = !!(await getAwaitingTableData(conversationId));
      const skipOneDrive = isConfirmWord || hasPending || hasAwaiting;

      if (!fileTsv && aadObjectId && !skipOneDrive) {
        try {
