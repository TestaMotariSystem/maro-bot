...
export function isFormatConfirm(text: string): boolean {
  ...
  // Exact match
  if (["yes", "save", "ja", "speichern"].includes(t)) return true;
  // Also match when mixed with greetings or other short text (e.g. "Hi save", "ok yes")
  return /\b(save|yes|ja|speichern)\b/i.test(t) && t.split(/\s+/).length <= 4;
}

export function isFormatCancel(text: string): boolean {
  const t = (text || "").toLowerCase().trim();
  if (["no", "cancel", "discard", "abbrechen", "nein"].includes(t)) return true;
  return /\b(cancel|discard|abbrechen|nein)\b/i.test(t) && t.split(/\s+/).length <= 4;
}
