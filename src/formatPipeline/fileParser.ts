...
/** Parse an Excel buffer (.xlsx/.xls) into TSV text. Reads ALL sheets, separated by sheet name headers. */
function excelBufferToTsv(buf: Buffer, fileName: string): string {
  const workbook = XLSX.read(buf, { type: "buffer" });

  if (workbook.SheetNames.length === 0) throw new Error(`No sheets found in "${fileName}".`);
